 ...
 * Richard Ding (riding)
 *
 * Source file with main functions used
...
void loadFile(struct info *imgInfo, struct f *file){
   ...
   if (s){
      ...
      for (i=0; i< DIRECT_ZONES && s>0; i++){
         ...
         /* decrement data left to be stored */
         s -= imgInfo->zonesize;
         /* move over to next place to store */
      ...
      if (s>0){
         if (file->node.indirect){
            ...
            for (i=0; i<zones && s>0; i++){
               memset(point, 0, imgInfo->zonesize);
               s-=imgInfo->zonesize;
               point+=imgInfo->zonesize;
            }
         }
      ...
      if (s>0){
         ...
         if (file->node.two_indirect){
            ...
            /* go through zone numbers of zone numbers to get zones */
            for (j=0; j< zones&&s>0; j++){
               if (di_zone[j]){
                  /* copy over zone numbers to get zones */
                  fseek(imgInfo->f,
                  ...
                  /* go through zone numbers to get zones */
                  for (i=0; i< zones &&s>0; i++){
                     if (s < imgInfo->zonesize){
                        move = s;
                     }
                     ...
                     if (curr_zone_two[i]){
                        offset=
                           imgInfo->place+imgInfo->zonesize*curr_zone_two[i];
                        fseek(imgInfo->f, offset, SEEK_SET);
                        fread((void*)hold, move, 1, imgInfo->f);
                        memcpy(point, hold, move);
                     }
                     ...
                     /* decrement data needed to store */
                     s-=imgInfo->zonesize;
                     /* increment pointer of place stored */
                     point+=imgInfo->zonesize;
                  }
               }
               else{
